...
public class FanClubDB
{
    ...
    {
        //*************************************************************************************
        // TODO 11: Construct the SQL statement to retrieve the id, name, time, venue, member *
        //          fee and nonmember fee of the current events a registered user, identified *
        //          by a user name, HAS JOINED. Order the result by event name. A current     *
        //          event is one whose date is greater than or equal to today's date.         *
        //*************************************************************************************
        sql = "";
        return myOracleDBAccess.GetData(sql);
    }
    ...
    {
        //******************************************************************************************************
        // TODO 12: Construct the SQL statement to retrieve the id, name, time, venue, member fee and          *
        //          nonmember fee of the past events a registered user, identified by a user name, HAS JOINED. *
        //          Order the result by event name. A past event is one whose date is less than today's date.  *
        //******************************************************************************************************
        sql = "";
        return myOracleDBAccess.GetData(sql);
    }
        ...
        string memberFee, string nonmemberFee, string eventQuota, string isAvailable, string isMemberOnly, string employeeId)
    {
        ...
        if (myOracleDBAccess.SetData(sql, trans))
        {
            ...
            {
                //************************************************************************************************
                // TODO 18: Construct the SQL statement to relate the new event to a fan club holding the event. *
                //************************************************************************************************
                sql = "";
                if (!myOracleDBAccess.SetData(sql, trans))
